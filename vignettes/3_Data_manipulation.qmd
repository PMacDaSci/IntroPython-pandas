...
Removes leading and trailing whitespace from each string in the Series.
...
```python
df['address'] = df['address'].str.replace('Street', 'St')  # Replace "Street" with "St"
```
...
```python
df['first_name'] = df['name'].str.split().str[0]  # Split the name column and take the first part
df['full_name'] = df[['first_name', 'last_name']].apply(' '.join, axis=1)  # Join first and last names
```

### `str.extract()`
- Extracts substrings based on a regular expression pattern.

```python
df['area_code'] = df['phone_number'].str.extract(r'(\d{3})')  # Extract the first 3 digits as the area code
```

### `str.find()` and `str.index()`
- `str.find()` returns the lowest index of the substring; returns `-1` if not found.
- `str.index()` works similarly but raises a `ValueError` if the substring is not found.
...
```python
df['zip_code'] = df['zip_code'].str.zfill(5)  # Ensure all zip codes have 5 digits
```
